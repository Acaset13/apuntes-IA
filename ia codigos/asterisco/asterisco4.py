...
# ---------------- Configuración principal (se ajusta aquí filas y columnas) ----------------
ANCHO_VENTANA = 600
...
FILAS = 11               # número de filas 
COLS = 11                # número de columnas
# --------------------------------------------------------------------------------------
...
class Nodo:
        ...
        for dr, dc in direcciones:
            ...
            if 0 <= nf < self.filas and 0 <= nc < self.cols:
                ...
                if abs(dr) == 1 and abs(dc) == 1:
                    orto1 = grid[self.fila][nc]   # misma fila, col del vecino
    ...
    def __eq__(self, otro):
        # proteger contra comparación con None u otros tipos
...
class AStar:
    ...
    def step(self):
        ...
        # quitar de open_order_list si está
...
def main():
    ...
    info_msg = "Click izq: toggle muro | Click der: restablecer | S: colocar inicio | E: colocar fin"
    ...
    while corriendo:
        ...
        # Mostrar mensaje de ayuda en footer (segunda línea)
        ayuda_text = FUENTE_FOOTER.render(info_msg, True, NEGRO)
        VENTANA.blit(ayuda_text, (10, ANCHO_VENTANA + MARGEN_SUPERIOR + 44))
        pygame.display.update()
        ...
        for evento in pygame.event.get():
            ...
            # manejo de teclas para colocar inicio/fin en la celda donde esté el mouse
            if evento.type == pygame.KEYDOWN:
                mouse_pos = pygame.mouse.get_pos()
                click = obtener_click_pos(mouse_pos, FILAS, COLS, ancho_total)
                if click:
                    f, c = click
                    nodo = grid[f][c]
                    if evento.key == pygame.K_s:  # 'S' -> colocar inicio
                        # si había un inicio previo, restablecerlo
                        if inicio is not None:
                            inicio.restablecer()
                        inicio = nodo
                        inicio.hacer_inicio()
                    elif evento.key == pygame.K_e:  # 'E' -> colocar fin
                        if fin is not None:
                            fin.restablecer()
                        fin = nodo
                        fin.hacer_fin()
            ...
            if evento.type == pygame.MOUSEBUTTONDOWN:
                ...
                if click:
                    ...
                    # BOTÓN IZQUIERDO: TOGGLE muro (puedes marcar y desmarcar libremente)
                    if evento.button == 1:
                        # Si la casilla es inicio o fin, no la toggles; usa S/E para posicionarlos
                        if nodo.es_inicio() or nodo.es_fin():
                            pass
                        else:
                            if nodo.es_pared():
                                nodo.restablecer()  # desmarcar
                            else:
                                nodo.hacer_pared()   # marcar

                    # BOTÓN DERECHO: restablecer (como antes)
